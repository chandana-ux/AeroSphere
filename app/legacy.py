...
def render_legacy(page, ROOT, code_root):
    try:
        ...
        st.error('Mission artifacts are incomplete. Analyze the flight or open a completed mission.')
        st.stop()
    ...
    if page in ('Quality & Coverage','Semantic AI','Process New Input'):
    ...
    elif page == 'Evidence & Replay':
        render_lineage(ROOT,summary,selection)
    elif page == 'Overview':
        ...
        offline_chart(chart_style(fig,420),width='stretch')
        if geo['status']=='provisional_horizontal_fit':
            ...
            offline_chart(chart_style(fig),width='stretch')
        ...
        st.info('Absolute altitude and metre-scaled 3D model measurements are not established. Raw altitude values are not used as a validated vertical reference.')
        ...
        offline_chart(chart_style(fig,550),width='stretch',config={'displaylogo':False})
        ...
        if kind=='GPS camera positions':
            ...
            offline_chart(chart_style(fig,350),width='stretch')
